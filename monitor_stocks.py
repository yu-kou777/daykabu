...
if __name__ == "__main__":
    ...
    requests.post(DISCORD_WEBHOOK_URL, json={"content": f"🚀 **プライム市場({len(ticker_list)}社) 合体版哨戒を開始** ({now_str})"})

    # 2. 一括データ取得（高速版）
    # 1600件あるので、ここが一番時間がかかります（約5〜10分）
    ...
    requests.post(DISCORD_WEBHOOK_URL, json={"content": f"✅ **本日のパトロール完了** 合致: {found_count}件"})
